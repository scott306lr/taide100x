# Source: https://github.com/huggingface/transformers/blob/e0d82534cc95b582ab072c1bbc060852ba7f9d51/src/transformers/models/llama/modeling_llama.py
...
# [MODIFIED] Import functions from transformer library
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.inv_freq = 1.0 / (base ** (torch.arange(0, head_dim, 2, device=device, dtype=torch.float32) / head_dim))
    ...
    def get_cos_sin(self, seq_lens: torch.Tensor, device, dtype, is_prefill):
        """
        ...
        self._update_cos_sin_cache(dtype, position_ids.device, max_seq_len)
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        #     (self.gate_proj.weight.data, self.up_proj.weight.data), dim=0
        # )
        # self.swiglu.w3.weight.data = self.down_proj.weight.data
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, flashinferWrapper: FlashinferAttentionWrapper ,config: LlamaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        if layer_idx is None:
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = self.q_proj(hidden_states).contiguous()
        key_states = self.k_proj(hidden_states).contiguous()
        value_states = self.v_proj(hidden_states).contiguous()  

        q, k, v = self.flashinferWrapper.reshape_qkv_for_attention(
            query_states, key_states, value_states, batch_position
        )
        
        if position_embeddings is None:
            raise ValueError("position_embeddings is None")
        else :
            cos, sin = position_embeddings
        ...
        return attn_outputs, attn_weights, past_key_value
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, flashinferWrapper: FlashinferAttentionWrapper):
        super().__init__()
        self.hidden_size = config.hidden_size

        # MODIFIED 
        self.self_attn = LlamaAttention(flashinferWrapper=flashinferWrapper, config=config, layer_idx=layer_idx)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, layer_idx , self.flashinferWrapper) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        # for flashinfer
        kvCachePool: KvCachePool = None,
        is_prefill: bool = False,
        batch_position: KvCacheBatchPosition = None,
        ...
        position_embeddings = self.rotary_emb.get_cos_sin(
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                raise Exception("Sorry, only support inference")
